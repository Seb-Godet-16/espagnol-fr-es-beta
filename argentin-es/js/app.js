/* ========================================
   Español Argentino → Français 🇦🇷 – Logique applicative
...
var done=[];
function loadDone(){try{done=JSON.parse(localStorage.getItem('pe_ar_done_v1')||'[]');}catch(e){done=[];}};
function saveDone(){try{localStorage.setItem('pe_ar_done_v1',JSON.stringify(done));}catch(e){}}
  ...
  ['grid1','grid2'].forEach(function(gid){
    ...
    document.getElementById(gid).innerHTML=ALL_THEMES.filter(function(t){return t.level===lv;}).map(function(t){
      return '<div class="theme-card'+(isDone(t.id)?' done':'')+'" onclick="openTheme(\''+t.id+'\')">'
        ...
        +(isDone(t.id)?'<button onclick="event.stopPropagation();resetTheme(\'\\\''+t.id+'\\\'\''+')" style="margin-top:6px;font-size:.65rem;background:#fff;border:1.5px solid #74ACDF;color:#74ACDF;border-radius:50px;padding:4px 10px;cursor:pointer;font-weight:700">🔄 Reiniciar</button>':'')
        +'</div>';
    }).join('');
  });
}
...
function renderFlash(){
  ...
  if(hasConj){
    ...
    frontContent=emFr+'<div class="fc-front-word">'+card.es+'</div><div class="fc-front-hint">👆 Tocá para ver la traducción</div>';
    backContent=emBk+'<div class="fc-back-word">'+card.fr+'</div>';
  }
  document.getElementById('tabContent').innerHTML=
    '<div class="section-label">Recto: español 🇦🇷 — Verso: français 🇫🇷 · ¡Tocá para voltear!</div>'
    +'<div class="fc-wrap">'
...
function buildAlphaDetail(c){
  return '<div style="font-size:2.5rem;font-weight:900;color:#74ACDF">'+c.es+'</div>'
    +'<div style="color:#555;margin:4px 0;font-size:.85rem">'+c.fr+'</div>'
    +'<button onclick="speak(\''+esc(c.es)+'\')" style="margin-top:10px;background:#74ACDF;color:#fff;border:none;border-radius:50px;padding:9px 18px;cursor:pointer;font-weight:700;min-height:44px">🔊 Escuchar</button>';
}
...
function renderQuiz10(){
  ...
  if(q10Step>=total){
    ...
    document.getElementById('tabContent').innerHTML='<div class="result-box">'
      ...
      +'<div style="font-size:1rem;margin:6px 0;color:'+(pct===100?'#4CAF50':'#74ACDF')+'">'
      +(pct===100?'¡Módulo desbloqueado! ⭐':'Necesitás 100% para validar. ¡Intentalo de nuevo!')+' </div>'
...
function checkQ10(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ ¡Correcto! ¡Qué piola!':'❌ Respuesta correcta: '+correctWord;
...
function renderVocab(){
  ...
  document.getElementById('tabContent').innerHTML=
    ...
    +'<div class="vocab-title">📚 Vocabulario clave — ¡Tocá para escuchar!</div>'
...
function renderDialogQuiz(){
  ...
  if(dqStep>=total){
    ...
    document.getElementById('tabContent').innerHTML='<div class="result-box">'
      +'<div style="font-size:3rem">'+(pct===100?'🎉':'💪')+'</div>'
      +'<h3>'+(pct===100?'¡Perfecto! ✅':'¡Seguí practicando!')+'</h3>'
      +'<div class="score-num">'+dqScore+'/'+total+'</div>'
      +'<div style="font-size:.9rem;margin-top:6px;color:'+(pct===100?'#4CAF50':'#74ACDF')+'">'
      +(pct===100?'¡Módulo desbloqueado! ⭐':'Necesitás 100% para validar. ¡Intentalo de nuevo!')+' </div>'
...
function checkDQ(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ ¡Correcto! ¡Qué piola!':'❌ ¡Intentalo de nuevo!';
  fb.style.color=chosen===correct?'#4CAF50':'#c0392b';
