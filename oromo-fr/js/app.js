/* ========================================
   Français 🇫🇷 → Afaan Oromoo 🇪🇹 (Pour apprenants Oromos)
   Logique applicative — Français au Recto / Oromo au Verso
...
function speak(txt) {
  ...
  function speakPart(i) {
    ...
    u.lang = 'fr-FR'; // CONFIGURATION : Moteur vocal en Français
    u.rate = 0.80;    // Un poil plus lent pour faciliter l'écoute pédagogique
    u.onend = function() {
      if (i + 1 < parts.length) {
...
var done=[];
function loadDone(){try{done=JSON.parse(localStorage.getItem('pe_om_fr_done_v1')||'[]');}catch(e){done=[];}}
function saveDone(){try{localStorage.setItem('pe_om_fr_done_v1',JSON.stringify(done));}catch(e){}}
...
function openTheme(id){
  ...
  var tabs;
  if(CT.type==='dialog'){
    tabs=[{k:'dialog',lbl:'💬 Maree'},{k:'vocab',lbl:'📚 Jechoota'},{k:'dquiz',lbl:'❓ Gaaffilee'}];
  } else if(CT.type==='alpha'){
    tabs=[{k:'flash',lbl:'🔤 Qubee'},{k:'quiz10',lbl:'🔊 Quiz Sagalee'}];
  } else {
    tabs=[{k:'flash',lbl:'🃏 Kaardota'},{k:'quiz10',lbl:'❓ Gaaffilee'}];
  }
...
function buildAlphaDetail(c){
  return '<div style="font-size:2.5rem;font-weight:900;color:#009A44">'+c.fr+'</div>'
    +'<div style="color:#555;margin:4px 0;font-size:.85rem">'+c.es+'</div>'
    +'<button onclick="speak(\''+esc(c.fr)+'\')" style="margin-top:10px;background:#009A44;color:#fff;border:none;border-radius:50px;padding:9px 18px;cursor:pointer;font-weight:700;min-height:44px">🔊 Dhaggeeffadhu</button>';
}
...
function renderQuiz10(){
  ...
  var total=qs.length;
  if(!qs||!total){
    document.getElementById('tabContent').innerHTML='<div class="result-box"><p>Gaffiinkoo hin jiru.</p></div>';
    return;
  }
  if(q10Step>=total){
    ...
    document.getElementById('tabContent').innerHTML='<div class="result-box">'
      ...
      +'<h3>'+(pct===100?'Baayʼee gaari da! ✅':'Quiz xumurameera!')+'</h3>'
      +'<div class="score-num">'+q10Score+'/'+total+'</div>'
      +'<div style="font-size:1rem;margin:6px 0;color:'+(pct===100?'#009A44':'#EF2B2D')+'">'
      +(pct===100?'Kutaan kun milkiin darbeera! ⭐':'Darbuuf 100% barbaachisa. Deebisii yaali!')+' </div>'
      +'<div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:14px">'
      +'<button class="retry-btn" style="background:#888" onclick="q10Step=0;q10Score=0;q10Answered=false;renderQuiz10()">🔄 Deebisi yaali</button>'
  ...
  if(isAlphaQuiz()){
    ...
    document.getElementById('tabContent').innerHTML=
      ...
      +'<div class="alpha-audio-label">Sagalee dhaggeeffadhu kutaa sirrii filadhu<br><small>Gaaffii '+(q10Step+1)+'/'+total+'</small></div>'
...
function checkQ10(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ Sirrii dha! Baga gammadde!':'❌ Dogoggora. Deebiin sirriin: '+correctWord;
...
function renderDialog(){
  ...
  document.getElementById('tabContent').innerHTML=
    ...
    +'<button class="btn-start-quiz" onclick="switchTab(\'dquiz\')">Quiz jalqabi ➜</button>'
    +'</div>';
...
function renderVocab(){
  ...
  document.getElementById('tabContent').innerHTML=
    ...
    +'<div class="vocab-title">📚 Jechoota murteessoo — Sagalee dhaggeeffachuuf cuqaasi !</div>'
...
function renderDialogQuiz(){
  ...
  if(dqStep>=total){
    ...
    document.getElementById('tabContent').innerHTML='<div class="result-box">'
      +'<div style="font-size:3rem">'+(pct===100?'🎉':'💪')+'</div>'
      +'<h3>'+(pct===100?'Guutuu dha! ✅':'Shaakala xumurameera!')+'</h3>'
      +'<div class="score-num">'+dqScore+'/'+total+'</div>'
      +'<div style="font-size:.9rem;margin-top:6px;color:'+(pct===100?'#009A44':'#EF2B2D')+'">'
      +(pct===100?'Kutaan kun banameera! ⭐':'Darbuuf 100% deebii sirrii barbaachisa. Deebisii yaali!')+' </div>'
      +'<div style="display:flex;gap:8px;justify-content:center;margin-top:14px;flex-wrap:wrap">'
      +'<button class="retry-btn" style="background:#888" onclick="dqStep=0;dqScore=0;dqAnswered=false;renderDialogQuiz()">🔄 Deebisi yaali</button>'
      +(pct===100?'<button class="retry-btn" onclick="renderSections();showScreen(\'sections\')">✓ Xumuri</button>':'')
      +'</div></div>';
    renderSections();return;
  }
...
function checkDQ(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ Deebii sirrii dha!':'❌ Deebistee yaali!';
  fb.style.color=chosen===correct?'#009A44':'#c0392b';
