/* ========================================
   Español Castillan → Français 🇪🇸 – Logique applicative
...
var done=[];
function loadDone(){try{done=JSON.parse(localStorage.getItem('pe_es_done_v1')||'[]');}catch(e){done=[];}}
function saveDone(){try{localStorage.setItem('pe_es_done_v1',JSON.stringify(done));}catch(e){}}
...
function buildAlphaDetail(c){
  return '<div style="font-size:2.5rem;font-weight:900;color:#AA151B">'+c.es+'</div>'
    ...
    +'<button onclick="speak(\''+esc(c.es)+'\')" style="margin-top:10px;background:#AA151B;color:#fff;border:none;border-radius:50px;padding:9px 18px;cursor:pointer;font-weight:700;min-height:44px">🔊 Escuchar</button>';
}
...
function checkQ10(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ ¡Correcto! ¡Qué guay!':'❌ Respuesta correcta: '+correctWord;
...
function checkDQ(chosen,correct){
  ...
  fb.textContent=chosen===correct?'✅ ¡Correcto! ¡Qué guay!':'❌ ¡Inténtalo de nuevo!';
  fb.style.color=chosen===correct?'#4CAF50':'#c0392b';
